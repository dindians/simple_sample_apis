...
import org.example.simple_sample_apis.app.usecase_factories.CustomerUsecaseFactory;
...
public final class CustomersController {
  ...
  public CustomersController(@NotNull CustomerUsecaseFactory customerUsecaseFactory) { getCustomerUsecase = customerUsecaseFactory.getCustomerUsecase(); }
  ...
  public Mono<String> getCustomer(@NotNull ServerHttpResponse serverHttpResponse, @PathVariable Integer customerId) {
    Function<EitherError, String> failureResponse = eitherError -> {
      ...
      return EitherErrorToJson.execute(eitherError, String.format("rest-controller-class: %s; path: /persons/%d", CustomersController.class.getName(), customerId), CustomersController.class);
    };
